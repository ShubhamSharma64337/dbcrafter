...
export default function TemplateModal({theme, visible, toggleVisible, templates, setIsLoading, setDiagram, urls, showAlert}) {
  ...
  return (
    visible && templates && <div className={`overlay overflow-auto fixed justify-center flex items-start p-5 top-0 left-0 w-screen h-screen bg-black bg-opacity-75`}>
        <div className={`modal ${theme==='dark'?'bg-gray-900 text-white':'bg-white'} rounded w-full`}>
            {/* Modal Header */}
            ...
            <div className="modal-body grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-5 p-5">
              {templates ? templates.map((element, index) => {
                return <button key={index} className={`card grid grid-cols-1 grid-rows-4  rounded p-1 transition shadow hover:shadow-none border-2 ${theme==='dark'?'border-gray-700 hover:border-blue-400':'border-slate-200 hover:border-blue-500 '}`} onClick={openTemplate} data-id={element._id}>
                  ...
                  <div className='row-span-1  rounded py-2'>
                    {element.name}
                  </div>
                </button>
              })
