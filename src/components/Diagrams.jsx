    ...
    diagrams ? <div className='p-5 grid gap-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
          {diagrams.map((element, index)=>{
              ...
              <div key={index} className="card p-1 rounded-lg border-2 border-blue-50 shadow-lg flex flex-col justify-center items-center transition overflow-hidden hover:shadow hover:border-2 hover:border-blue-400">
                  <div className="image flex justify-center items-center px-2 py-10 w-full" data-diagramname={element.name} data-diagramid={element._id}  onClick={!element.isEditing?openDiagram:undefined}>
                  ...
                  <div className="body rounded bg-blue-50 flex justify-between items-center w-full overflow-x-auto p-3">
                      ...
                      But, if the key value exists, and changes on rerender, the defaultValue also changes */}
                        <input key={element.name+index} type='text' required={true} id={element.name+'Input'} defaultValue={element.name} className="diagram-name rounded px-1 py-0.5 outline-blue-700 enabled:border disabled:bg-blue-50" disabled={element.isEditing?false:true} data-diagramname={element.name}/>
                      </form>
                      <div className='right buttons flex flex-nowrap'>
                        ...
                        <button className={`delete-button rounded-full p-2 hover:bg-red-200 ${element.isEditing?'hidden':''}`} data-diagramname={element.name} data-diagramid={element._id} onClick={deleteDiagram}>
