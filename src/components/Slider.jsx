...
export default function Slider({theme}) {
  ...
  const captions = ['Simple, easy to understand diagrams','Create and store as many diagrams as you want','One click conversion to SQL']
    useEffect(()=>{
        if(currentImage===1){
  ...
  return (
          <div className="slider-large flex flex-col sm:flex-row gap-x-5 items-center justify-center">
              ...
              <figure className='flex flex-col justify-center items-center w-4/5 sm:w-3/5'>
                <img id='sliderImage' src={img1} className={`hover:scale-95 transition w-full h-auto rounded-lg border-2 ${theme==='dark'?'border-blue-500':'border-slate-500'}`}></img>
